...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class OrdersController : BaseController
    {
        ...
        public ActionResult OrderDetails(int? id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    {
                        Notification.set_flash("Không tồn tại đơn hàng: " + "Gốm sứ Bát Tràng" + id + ")", "warning");
            ...
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
                {
                    ...
                    if (order != null)
                    {
                        ...
                        db.Entry(order).State = EntityState.Modified;
                        db.SaveChanges();
                        Notification.set_flash("Đã chuyển trạng thái đơn hàng: " + "Gốm sứ Bát Tràng" + id + " sang chờ xử lý!", "success");
                    }
                    else
                    {
        ...
        //Chuyển trạng thái sang đóng gói
        public ActionResult ChangeProcessing1(string ButtonConfirmlink, int? id, string ProductOrder, string SubOrderTotal, string Discount_Price, string OrderTotal, string OrderAddress)
        {
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
                {
                    ...
                    string OrderPayment = db.Payments.Where(m => m.payment_id == order.payment_id).FirstOrDefault().payment_name;
                    string OrderStatus = "<span style='color:#17a2b8;'>Đang đóng gói</span>";
                    ButtonConfirmlink = Request.Url.Scheme + "://" + Request.Url.Authority + "/account/order_detail/" + order.order_id;
                    SendEmailOrders(ButtonConfirmlink, Discount_Price, SubOrderTotal, ProductOrder, OrderPayment, OrderStatus, emailID, OrderID, OrderTotal,
                    OrderPhone, OrderName, OrderAddress, OrderDelivery, DeliveryId, "ChangeProcessing1");
                    db.SaveChanges();
                    Notification.set_flash("Đã chuyển trạng thái đơn hàng: " + "#" + id + " sang đang đóng gói!", "success");
                    return RedirectToAction("OrderIndex");
                }
                else
                {
        ...
        public ActionResult ChangeProcessing2(string ButtonConfirmlink, int? id, string ProductOrder, string SubOrderTotal, string Discount_Price, string OrderTotal, string OrderAddress)
        {
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
                {
                    ...
                    string OrderPayment = db.Payments.Where(m => m.payment_id == order.payment_id).FirstOrDefault().payment_name;
                    string OrderStatus = "<span style='color:#17a2b8;'>Đang vận chuyển</span>";
                    ButtonConfirmlink = Request.Url.Scheme + "://" + Request.Url.Authority + "/account/order_detail/" + order.order_id;
                    SendEmailOrders(ButtonConfirmlink, Discount_Price, SubOrderTotal, ProductOrder, OrderPayment, OrderStatus, emailID, OrderID, OrderTotal,
                    OrderPhone, OrderName, OrderAddress, OrderDelivery, DeliveryId, "ChangeProcessing2");
                    db.SaveChanges();
                    Notification.set_flash("Đã chuyển trạng thái đơn hàng: " + "#" + id + " sang đang vận chuyển!", "success");
                    return RedirectToAction("OrderIndex");
                }
                else
                {
        ...
        string OrderPhone,string OrderName,string OrderAddress,string OrderDelivery,string DeliveryId, string emailFor)
        {
            ...
            if (emailFor == "ChangeProcessing")
            {
                ...
                body = body.Replace("{{BodyContent}}", "Một kiện hàng thuộc đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được giao cho đơn vị đóng gói.");
            ...
            else if (emailFor == "ChangeProcessing1")
            {
                subject = "Đơn hàng #" + OrderID + " đang được đóng gói";
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Một kiện hàng thuộc đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được đóng gói và đang chuyển giao cho đơn vị vận chuyển.");
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink);
                body = body.Replace("{{UserEmail}}", emailID);
            ...
            else if (emailFor == "ChangeProcessing2")
            {
                subject = "Đơn hàng #" + OrderID + " đang được vận chuyển";
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Một kiện hàng thuộc đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đang dược vận chuyển.");
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink);
                body = body.Replace("{{UserEmail}}", emailID);
            ...
            else if (emailFor == "ChangeComplete")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được giao thành công. Cảm ơn bạn đã tin tưởng Gốm sứ Bát Tràng, hi vọng trong tương lai bạn sẽ tiếp tục sử dụng dịch vụ của Chúng tôi.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink); 
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
            ...
            else if (emailFor == "CancleOrders")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được hủy theo yêu cầu của bạn. Gốm sứ Bát Tràng bắt đầu tiến hành thủ tục hoàn tiền và sẽ thông tin đến bạn khi đã hoàn tất.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink);
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
        ...
        public void ExportDataToExcel(DateTime picker1 , DateTime picker2, string order__status,int payment__method)
        {
            ...
            using (ExcelEngine excelEngine = new ExcelEngine())
            {
                ...
                workbook.SaveAs("GomsuBatTrang_Report_"+ picker1.ToString("dd-MM-yyyy") + "_" + picker2.ToString("dd-MM-yyyy") + "_" + file + ".xlsx", ExcelSaveType.SaveAsXLS, HttpContext.ApplicationInstance.Response, ExcelDownloadType.Open);
            }
        }
